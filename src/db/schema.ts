...
import { pgTable, uuid, text, varchar, timestamp, boolean, integer, date, decimal, pgEnum } from 'drizzle-orm/pg-core';
...
export const parkingPermits = pgTable('parking_permits', {
  id: uuid('id').primaryKey().defaultRandom(),
  tenantId: uuid('tenant_id').references(() => tenants.userId, { onDelete: 'cascade' }),
